 ...
 */
public class WindowWithProcessFunctionNewAPI2<Key, IN, OUT>
    extends KeyedProcessFunction<Key, IN, OUT> implements ResultTypeQueryable<OUT> {

  private static final Logger LOG = LoggerFactory.getLogger(WindowWithProcessFunctionNewAPI2.class);
  ...
  public WindowWithProcessFunctionNewAPI2(
  ...
  public void processElement(IN value, Context ctx, Collector<OUT> out) throws Exception {
    ...
    for (TimeWindow window : windows) {
      ...
      // TODO: can we get rid of this state access?
      boolean firstInWindow = true;
  ...
  public void onTimer(long timestamp, OnTimerContext ctx, Collector<OUT> out) throws Exception {
    ...
    if (ctx.timeDomain() == TimeDomain.EVENT_TIME) {
      ...
      if (currentState != null) {
        emitWindowContents(out, new TimeWindow(0L, windowEndStateKey), currentState, ctx);
  ...
  public WindowWithProcessFunctionNewAPI2<Key, IN, OUT> produces(TypeInformation<OUT> producedType) {
    this.producedType = producedType;
    return this;
  }

  @SuppressWarnings({"unused", "UnusedReturnValue"})
  public WindowWithProcessFunctionNewAPI2<Key, IN, OUT> allowedLateness(Time lateness) {
  ...
  @SuppressWarnings({"unused", "UnusedReturnValue"})
  public WindowWithProcessFunctionNewAPI2<Key, IN, OUT> sideOutputLateData(OutputTag<IN> outputTag) {
  ...
  @SuppressWarnings({"unused", "UnusedReturnValue"})
  public WindowWithProcessFunctionNewAPI2<Key, IN, OUT> triggerMode(TriggerResult windowFireMode) {
    checkArgument(
